...
export default function resolveToProductPath(sourcePath: string): ?string {
	...
	if (parsedExtensionPath)
	{
		...
		return `/${root}/js/${jsDir}/${extension.join('/')}/${filePath}`;
	}
	...
	if (parsedComponentPath)
	{
		...
		return `/${root}/components/${namespace}/${component}/templates/${template}/${filePath}`;
	}
	...
	if (parsedSiteTemplatePath)
	{
		...
		return `/${root}/templates/${template}/${filePath}`;
	}
